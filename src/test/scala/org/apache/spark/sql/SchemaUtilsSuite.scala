...
import org.apache.spark.sql.SchemaUtils.SchemaEvolutionException
...
class SchemaUtilsSuite extends FunSuite{
  ...
  test("simple schema, no changes") {
    ...
    val tgtRows = SchemaUtils.schemaEvolution(srcRows.iterator, srcSchema, srcSchema).toSeq
  ...
  test("simple schema, new field") {
    ...
    val tgtRows = SchemaUtils.schemaEvolution(srcRows.iterator, srcSchema, tgtSchema).toSeq
  ...
  test("simple schema, unsupported data type change") {
    ...
    intercept[SchemaEvolutionException]{
      SchemaUtils.schemaEvolution(srcRows.iterator, srcSchema, tgtSchema).toSeq
    }
  }
  ...
  test("nested schema, new field with default value") {
    ...
    val tgtRows = SchemaUtils.schemaEvolution(srcRows.iterator, srcSchema, tgtSchema, Map("nested.c1" -> expr("(nested.a1 + 9) - a"))).toSeq
  ...
  test("nested schema, new field with default value, wrong type") {
    ...
    intercept[AssertionError]{
      SchemaUtils.schemaEvolution(srcRows.iterator, srcSchema, tgtSchema, Map("nested.c1" -> expr("(nested.a1 + 9) - a")) ).toSeq
    }
  }
  ...
  test("evaluate nested expression") {
    ...
    val expr = SchemaUtils.prepareExpr( col("nested.a1"), schema)
